import { assert, assertEquals, assertStringIncludes } from "../../test/asserts.ts";
...
Deno.test("profile", async (t) => {
    ...
    const shareKeypair = await Earthstar.Crypto.generateShareKeypair('testing') as Earthstar.ShareKeypair;
    const authorKeypair = await Earthstar.Crypto.generateAuthorKeypair('test') as Earthstar.AuthorKeypair;
    const settings: Earthstar.SharedSettings = new Earthstar.SharedSettings({ namespace: NAMESPACE });
    ...
    await t.step({
        ...
        fn: async () => {
            const result = await profile.setDisplayName({ settings, replica, name: expected });
    ...
    await t.step({
        ...
        fn: async () => {
            ...
            const result = await profile.setStatus({ settings, replica, status: expected });
            assert(Earthstar.notErr(result));
        }
    });
